...
class Keypoint(TypedDict):
    id: Joint_ID
    is_visible: NotRequired[Union[int, np.ndarray]] # O or 1, but cases `array([], dtype=uint8)` are present ...
    x: float
    y: float
...
class Body_annotation(TypedDict):
    x1: NotRequired[int]
    y1: NotRequired[int]
    x2: NotRequired[int]
    y2: NotRequired[int]
    scale: NotRequired[Union[int,float,np.ndarray]] # int or float or `array([], dtype=float64)`
    objpos: NotRequired[Union[Point, np.ndarray]] # Point or `array([], dtype=float64)`
    annopoints: NotRequired[Union[Keypoint_annotation, np.ndarray]] # Keypoint_annotation or `array([], dtype=uint8)`
...
class Img_annotation(TypedDict):
    image: Img_name 
    annorect: Union[List[Body_annotation], Body_annotation, np.ndarray] # if np.ndarray ---> `array([], dtype=uint8)`
    vididx: Union[int, np.ndarray] # int 1-2821 OR `array([], dtype=float64)`
    frame_sec: Union[int, np.ndarray] # int OR `array([], dtype=float64)`

class Activity_annotation(TypedDict):
    act_name: Union[str, np.ndarray] # ''horse racing', 'driving automobile or light truck', ... OR empty `array([], dtype='<U1')`
    cat_name: Union[str, np.ndarray] # 'dancing', 'sports', ... OR empty `array([], dtype='<U1')`
...
class File_Annotation(TypedDict):
    ...
    img_train: np.darray # (24987, ) only 0 or 1
    version: str
    single_person: np.ndarray # (24987, ) dtype: object --> Union[int, np.ndarray 1D of contiguous int starting at 1]
